        ...
        if identifier then
            ...
            if lobby then
                ...
                for _, player in ipairs(lobby.players) do
                    TriggerClientEvent('insane-electrician:client:TakeLooby', tonumber(player["source"]))
                end
...
RegisterServerEvent('insane-electrician:loadData', function()
    local src = source
...
Citizen.CreateThread(function()
    ...
    RegisterCallback('insane-electrician:server:getPlayerData', function(source, cb)
        local src = source
        local identifier = GetIdentifier(src)
        local data = playerJobData[identifier]
        local dataInfo = {
            playerMoney = GetPlayerMoney(src, 'cash'),
    ...
    RegisterCallback('insane-electrician:server:checkAndPayDeposit', function(source, cb)
        local depositValue = Config.VehicleDeposit
        local cash  = GetPlayerMoney(source, 'cash')
        local bank  = GetPlayerMoney(source, 'bank')

        if (cash + bank) < depositValue then
            -- Não tem dinheiro suficiente
            local msg = string.format(Config.NotificationText['notEnoughDeposit'].text, depositValue)
            TriggerClientEvent('insane-electrician:client:sendNotification', source, msg,
                Config.NotificationText['notEnoughDeposit'].type)
            cb(false)
            return
        end

        -- Retirar do cash primeiro; se não chegar, tira o resto do bank
        if cash >= depositValue then
            RemoveMoney(source, 'cash', depositValue)
        else
            local remainder = depositValue - cash
            RemoveMoney(source, 'cash', cash)
            RemoveMoney(source, 'bank', remainder)
        end

        -- Guardar a caução no lobby do jogador para devolver depois
        local identifier = GetIdentifier(source)
        if not depositStorage then depositStorage = {} end
        depositStorage[identifier] = depositValue

        local msg = string.format(Config.NotificationText['depositPaid'].text, depositValue)
        TriggerClientEvent('insane-electrician:client:sendNotification', source, msg,
            Config.NotificationText['depositPaid'].type)

        cb(true)
    end)

    RegisterCallback('insane-electrician:server:getMissionsDetails', function(source, cb, data)
    ...
    RegisterCallback('insane-electrician:server:getVehicleData', function(source, cb, data)
        local src = source
    ...
    RegisterCallback("insane-electrician:server:HasItem", function(source, cb, item)
        cb(HasItem(source, item))
    end)

    RegisterCallback('insane-electrician:server:CreatePlayerLobby', function(source, cb)
        local src = source
        local identifier = GetIdentifier(src)
        local data = playerJobData[identifier]
        if not data then
    ...
    RegisterCallback('insane-electrician:server:getSellMarketData', function(source, cb)
        local src = source
    ...
    RegisterCallback("insane-electrician:canRemoveLift", function(source, cb, id)
        cb(CanPlayerLift(source, id))
    end)

    RegisterCallback("insane-electrician:placeLadder", function(source, cb, data)
        cb(true)
    ...
    RegisterCallback("insane-electrician:removeLadder", function(source, cb, data)
        local key = data.key
        local owneridentifier = data.owneridentifier
        local lobby = coopData[owneridentifier]
        if not lobby then return end
        ...
        SetTimeout(2000, function()
            ...
            TriggerClientEvent("insane-electrician:removeLadder", -1, key)
            for _, player in ipairs(lobby.players) do
                TriggerClientEvent('insane-electrician:client:UpdateLobby', player.source, lobby, 'ladder')
            end
        end)
    end)
    RegisterCallback('insane-electrician:server:getTrafoModal', function(source, cb, data)
        if not data then return end
        local src = source
        local owneridentifier = data.owneridentifier
...
RegisterServerEvent('insane-electrician:server:acceptInvite', function(data)
    local src = source
    local identifier = GetIdentifier(src)
    ...
    if lobby.roomSetting.startJob then
        TriggerClientEvent('insane-electrician:client:sendNotification', src,
    ...
    if #lobby.players >= 4 then
        TriggerClientEvent('insane-electrician:client:sendNotification', src, Config.NotificationText['lobbyfull'].text,
    ...
    for _, player in ipairs(lobby.players) do
        TriggerClientEvent('insane-electrician:client:RefreshLobby', player.source, coopData[hostidentifier])
    end
end)

RegisterServerEvent('insane-electrician:server:firePlayer', function(data)
    local src = source
    local identifier = GetIdentifier(src)
    local playerData = playerJobData[identifier]
    ...
    for _, player in ipairs(lobby.players) do
        TriggerClientEvent('insane-electrician:client:RefreshLobby', player.source, coopData[identifier])
    end
    TriggerClientEvent('insane-electrician:client:TakeLooby', oldPlayerID)
end)

RegisterServerEvent('insane-electrician:server:invetePlayer', function(data)
    local src = source
    local identifier = GetIdentifier(src)
    local playerData = playerJobData[identifier]
    local targetID = tonumber(data.targetID)
    ...
    TriggerClientEvent('insane-electrician:client:invetePlayer', targetID, playerData.profiledata.name, identifier,
        playerData.profiledata.source or src)
end)

RegisterServerEvent('insane-electrician:server:selectMission', function(data)
    local src = source
...
RegisterServerEvent('insane-electrician:server:startJob', function(data)
    local src = source
    local identifier = GetIdentifier(src)
    local playerData = playerJobData[identifier]
    local lobby = coopData[identifier]
    ...
    for _, player in ipairs(lobby.players) do
        Wait(200)
        TriggerClientEvent('insane-electrician:client:StartJobOwner', player.source, lobby, JoobTask[identifier])
    end
end)

RegisterServerEvent('insane-electrician:server:switchFixed', function(owneridentifier, coords)
    local src = source
    ...
    for index, value in ipairs(lobby.roomSetting.Mission.regiontrafficLampCoords) do
        if value.regionFixedTrafo.coords.x == coords.x and
            ...
            RollTaskReward(src, 'fixTrafficLamp')
            for _, player in ipairs(lobby.players) do
                TriggerClientEvent('insane-electrician:client:UpdateData', player.source,
...
RegisterServerEvent('insane-electrician:server:trafficLightsFixed', function(owneridentifier, coords)
    local src = source
    if not owneridentifier then return end
    local lobby = coopData[owneridentifier]
    if not lobby then return end
...
RegisterServerEvent('insane-electrician:server:switchFixedHouse', function(owneridentifier, coords)
    local src = source
    if not owneridentifier then return end
    local lobby = coopData[owneridentifier]
    if not lobby then return end
    ...
    for index, value in ipairs(lobby.roomSetting.Mission.regionhouseBoardCoords) do
        if value.coords.x == coords.x and value.coords.y == coords.y and value.coords.z == coords.z then
            ...
            RollTaskReward(src, 'fixHouseBoard')
            for _, player in ipairs(lobby.players) do
                TriggerClientEvent('insane-electrician:client:UpdateData', player.source,
...
RegisterServerEvent('insane-electrician:server:switchFixedTrafo', function(owneridentifier, coords)
    local src = source
    if not owneridentifier then return end
    local lobby = coopData[owneridentifier]
    if not lobby then return end
...
RegisterServerEvent('insane-electrician:server:switchFixedPhonePole', function(owneridentifier, coords)
    local src = source
    if not owneridentifier then return end
    local lobby = coopData[owneridentifier]
    if not lobby then return end
...
RegisterServerEvent('insane-electrician:server:switchFixedstreetLamp', function(owneridentifier, coords)
    local src = source
    if not owneridentifier then return end
    local lobby = coopData[owneridentifier]
    if not lobby then return end
    ...
    for index, value in ipairs(lobby.roomSetting.Mission.regionstreetLampCoords) do
        if value.coords.x == coords.x and value.coords.y == coords.y and value.coords.z == coords.z then
            ...
            RollTaskReward(src, 'fixStreetLamp')
            for _, player in ipairs(lobby.players) do
                TriggerClientEvent('insane-electrician:client:UpdateData', player.source,
...
RegisterServerEvent('insane-electrician:server:changeTrafoModal',
...
RegisterServerEvent('insane-electrician:server:UpdateJobTask', function(jobTaskName, owneridentifier)
    local src = source
    if not owneridentifier then return end
    local identifier = GetIdentifier(src)
    local lobby = coopData[owneridentifier]
            ...
            for k, v in pairs(JoobTask[owneridentifier].regionJobTask) do
                if v.jobName == jobTaskName then
                    ...
                    if v.madeAmount == v.jobCount then
                        v.finish = true
                        TriggerEvent('insane-electrician:server:FinishJob', v.jobName, identifier, owneridentifier)
                    end
    ...
    for _, player in ipairs(lobby.players) do
        TriggerClientEvent('insane-electrician:client:RefreshJob', player.source, JoobTask[owneridentifier])
    end
end)


RegisterServerEvent('insane-electrician:server:FinishJob', function(jobTaskName, identifier, owneridentifier)
    local src = source
    local lobby = coopData[owneridentifier]
    if not lobby then return end
    if not lobby.roomSetting.startJob then
    ...
    if allFinished then
        ...
        for _, player in ipairs(lobby.players) do
            TriggerClientEvent('insane-electrician:client:FinishJob', player.source, coopData[owneridentifier])
        end
    end
end)
...
RegisterServerEvent('insane-electrician:server:LeaveVehicle', function(owneridentifier)
    local src = source
    local identifier = GetIdentifier(src)
    local lobby = coopData[owneridentifier]
    if not lobby then return end
    ...
    for _, player in ipairs(lobby.players) do
        ...
        if #lobby.roomSetting.Vehicle == 0 then
            ...
            AddMoney(tonumber(player.source), 'cash', tonumber(lobby.roomSetting.regionAwards.money))

            -- Devolver caução APENAS ao dono do lobby
            if tonumber(player.source) == tonumber(lobby.roomSetting.ownersrc) then
                if depositStorage and depositStorage[owneridentifier] then
                    local depositValue = depositStorage[owneridentifier]
                    AddMoney(tonumber(lobby.roomSetting.ownersrc), 'cash', depositValue)
                    depositStorage[owneridentifier] = nil
                    TriggerClientEvent('insane-electrician:client:sendNotification', tonumber(lobby.roomSetting.ownersrc),
                        Config.NotificationText['depositReturned'].text,
                        Config.NotificationText['depositReturned'].type)
                end
            end
            ...
            for __, regionJobTask in ipairs(JoobTask[owneridentifier].Players) do
                if regionJobTask.identifier == player.identifier then
                    TriggerClientEvent('insane-electrician:client:LeaveVehicle', player.source, regionJobTask
...
function RollTaskReward(source, taskName)
    local rewardConfig = Config.TaskRewards and Config.TaskRewards[taskName]
    if not rewardConfig then return end

    if math.random(1, 100) > rewardConfig.chance then return end

    local selectedItem = rewardConfig.items[math.random(1, #rewardConfig.items)]
    if not selectedItem then return end

    addItem(source, selectedItem, rewardConfig.amount)
end
...
function AddXP(source, xp)
    ...
    if profiledata.level > #Config.RequiredXP then
        TriggerClientEvent('insane-electrician:client:sendNotification', src, Config.NotificationText['maxlevel'].text,
...
RegisterServerEvent('insane-electrician:server:resetJobButton', function(owneridentifier)
    local src = source
    local identifier = GetIdentifier(src)
    local owneridentifier = owneridentifier
    if not owneridentifier then return end
    if tostring(owneridentifier) ~= tostring(identifier) then
        TriggerClientEvent('insane-electrician:client:sendNotification', src, Config.NotificationText['notowner'].text,
        ...
        for _, player in ipairs(lobby.players) do
            if player.source == src then
                ...
                for _, remainingPlayer in ipairs(lobby.players) do
                    ...
                    TriggerClientEvent('insane-electrician:client:resetjob', remainingPlayer.source)
                    TriggerClientEvent('insane-electrician:client:sendNotification', remainingPlayer.source,
...
RegisterServerEvent('insane-electrician-server:build', function(modal, coords, owneridentifier)
    local src = source
    local lobby = coopData[owneridentifier]
    if not lobby then return end

    if modal == 'lift' then
        TriggerClientEvent("insane-electrician:createLift", src, coords)
        lobby.roomSetting.liftCraft = true
    elseif modal == 'ladder' then
        TriggerClientEvent("insane-electrician:createLadder", src, coords)
        lobby.roomSetting.ladderCraft = true
    end
    for _, player in ipairs(lobby.players) do
        TriggerClientEvent('insane-electrician:client:UpdateLobby', player.source, lobby)
    end
end)
...
function UpdateLift(id, target)
    TriggerClientEvent("insane-electrician:updateLift", target or -1, id or Lifts, id and Lifts[id] or nil)
end
...
RegisterNetEvent("insane-electrician:moveLift", function(id, direction, toggle, height)
    Lifts[id].moving = toggle
    Lifts[id].direction = (toggle and direction or nil)
    Lifts[id].currentHeight = height - 2.75
    UpdateLift(id)

    TriggerClientEvent("insane-electrician:moveLift", -1, id, direction, toggle, height)
end)

RegisterNetEvent("insane-electrician:spawnLift", function(coords, heading, railHeight)
...
RegisterNetEvent("insane-electrician:removeLift", function(id, owneridentifier)
    local source = source
    ...
    SetTimeout(1000, function()
        TriggerClientEvent("insane-electrician:removeLift", -1, id)
    end)
    SetTimeout(3000, function()
        local lobby = coopData[owneridentifier]
        if not lobby then return end
        ...
        for _, player in ipairs(lobby.players) do
            TriggerClientEvent('insane-electrician:client:UpdateLobby', player.source, lobby, 'lift')
        end
    end)
end)
...
function UpdateLadders(index, target)
    ...
    TriggerClientEvent("insane-electrician:updateLadders", target or -1, index or nil, data)
end
