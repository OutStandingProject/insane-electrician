        ...
        keyHandler(event) {
            if (event.keyCode == 27) {
                if (this.mainShow) {
                    ...
                    // Chama o reset interno do fourminigame para limpar timers e estado
                    if (window.fourminigameReset) window.fourminigameReset();
        ...
        eventHandler(event) {
            switch (event.data.action) {
                ...
                case "START_MINIGAME_CABLETWO":
                    // Hard-reset: limpa estado interno + DOM, depois reinicia como primeira vez
                    if (window.fourminigameReset) window.fourminigameReset();
                    this.foringShow = false;
                    this.$nextTick(() => {
...
var resourceName = "insane-electrician";
    ...
    function toggleLight(selector, visibility) {
        if (visibility) {
            ...
            if (completedLights[0] === 1 && completedLights[1] === 1 && completedLights[2] === 1 && completedLights[3] === 1) {
                $.post(
                    "https://insane-electrician/fixWirings",
                    JSON.stringify({
                        coords: value.coords,
...
// Variáveis de estado do fourminigame expostas globalmente para reset externo
var _fmg_currentGame = null;
var _fmg_currentTimer = null;
var _fmg_currentTimerLoop = null;
var _fmg_currentWeld = null;
var _fmg_doorStatus = false;

// Reset público: limpa todo o estado interno e o DOM sem precisar de recriar a função
window.fourminigameReset = function () {
    if (_fmg_currentTimerLoop) {
        clearInterval(_fmg_currentTimerLoop);
        _fmg_currentTimerLoop = null;
    }
    _fmg_currentGame = null;
    _fmg_currentTimer = null;
    _fmg_currentWeld = null;
    _fmg_doorStatus = false;
    $("body").css("cursor", "unset");
    $("#ams-welding").css("display", "none");
    $("#am-overlay").removeClass("active").css("background-image", `url("./img/hazard.png")`);
    $("#app-minigame").stop(true, true).hide();
    $("#ams-nodes").html("");
    $("#am-timer").html("");
};
...
window.fourminigame = function (eventdata, cbdata, endgamebool) {
    ...
    // Usa as variáveis globais para que o reset externo as consiga limpar
    var doorStatus = _fmg_doorStatus;
    var mouse = { x: 0, y: 0 };
    var wires = [];
    var currentWeld = _fmg_currentWeld;
    var currentTimer = _fmg_currentTimer;
    var currentTimerLoop = _fmg_currentTimerLoop;
    var currentGame = _fmg_currentGame;
    var audioInstances = {};

    function SetDoor(bool, cb) {
        doorStatus = bool;
        _fmg_doorStatus = bool;
        if (bool) {
    ...
    function StartWelding(wireIndex, direction) {
        ...
        $.post(
            "https://insane-electrician/welding",
            JSON.stringify({
                toggle: true
            })
        );
    ...
    function StartTimer(seconds) {
        ...
        _fmg_currentTimer = currentTimer;
        ...
        _fmg_currentTimerLoop = currentTimerLoop;
    }

    function StartGame(settings, cb) {
        // Não bloqueia mesmo que currentGame exista — o reset já foi feito antes
        currentGame = settings;
        _fmg_currentGame = currentGame;
    ...
    function OnGameCompleted(success) {
        $.post(
            "https://insane-electrician/gameCompleted",
            JSON.stringify({
                success: success
            })
        );
