...
function openElectricinMenu()
    if nuiLoaded and Core then
        local playerData = TriggerCallback('insane-electrician:server:getPlayerData')
        local ServerLobby = TriggerCallback('insane-electrician:server:CreatePlayerLobby')
        if playerData then
...
RegisterNetEvent('insane-electrician:client:invetePlayer', function(lobbyOwner, identifier)
    local playerData = TriggerCallback('insane-electrician:server:getPlayerData')
    if playerData then
        SetNuiFocus(true, true)
...
RegisterNetEvent('insane-electrician:client:RefreshLobby', function(lobbyData)
    local playerData = TriggerCallback('insane-electrician:server:getPlayerData')
...
RegisterNetEvent('insane-electrician:client:UpdateData', function(lobbyData, missionName)
        ...
        for index, value in ipairs(CoopDataClient.roomSetting.Mission.regiontrafficLampCoords) do
            ...
            for k, v in ipairs(value.regionTrafficCoords) do
                if v.fixed then
                    ...
                    local coordsKey = string.format("%.2f,%.2f,%.2f", v.coords.x, v.coords.y, v.coords.z)

                    if TrafficLamp2[coordsKey] then
                        RemoveBlip(TrafficLamp2[coordsKey])
                        TrafficLamp2[coordsKey] = nil
                    end
...
RegisterNetEvent("insane-electrician:client:StartJobOwner", function(data, data2) -- Start Job Owner
...
RegisterNetEvent('insane-electrician:client:StartJob', function(jobData, jobTask)
    if jobData.roomSetting.startJob then
        ...
        NuiMessage('START_JOB', joobTaskClient)
        SetNuiFocus(false, false)
        TriggerEvent('insane-electrician:server:FinishJob', CoopDataClient.roomSetting.owneridentifier)
    end
end)

RegisterNetEvent('insane-electrician:client:RefreshJob', function(jobTask)
    joobTaskClient = jobTask
    NuiMessage('REFRESH_JOBTASK', joobTaskClient)
end)

RegisterNetEvent('insane-electrician:client:UpdateLobby', function(lobbyData, ladderorlift)
    CoopDataClient = lobbyData
...
RegisterNetEvent('insane-electrician:client:TakeLooby', function()
    NuiMessage('CLOSENUI')
    SetNuiFocus(false, false)
    CoopDataClient = {}
end)
...
RegisterNUICallback('invitePlayer', function(data)
    TriggerServerEvent('insane-electrician:server:invetePlayer', data)
end)

RegisterNUICallback('firePlayer', function(data)
    TriggerServerEvent('insane-electrician:server:firePlayer', data)
end)

RegisterNUICallback('acceptInvite', function(data)
    SetNuiFocus(false, false)
    TriggerServerEvent('insane-electrician:server:acceptInvite', data)
end)

RegisterNUICallback('selectMission', function(data)
    TriggerServerEvent('insane-electrician:server:selectMission', data)
end)
...
RegisterNUICallback('startJob', function(data)
    ...
    if #distanceCheck == 0 and #distanceCheck2 == 0 then
        -- Verificar e cobrar caução antes de iniciar o job
        local depositResult = TriggerCallback('insane-electrician:server:checkAndPayDeposit')
        if not depositResult then
            return
        end
        TriggerServerEvent('insane-electrician:server:startJob', data)
    else
...
RegisterNUICallback('resetJob', function()
    TriggerServerEvent('insane-electrician:server:resetJobButton', CoopDataClient.roomSetting.owneridentifier)
end)

RegisterNUICallback('switchFixed', function(data)
    if not data.missionName then
        local missionPermit = TriggerCallback('insane-electrician:server:getMissionsDetails',
        ...
        if missionPermit then
            ...
            TriggerServerEvent('insane-electrician:server:switchFixed', CoopDataClient.roomSetting.owneridentifier,
...
RegisterNUICallback('switchFixedHouse', function(data)
    ...
    SetNuiFocus(false, false)
    NuiMessage('CLOSE_TRAFO_MISSION')
    TriggerServerEvent('insane-electrician:server:UpdateJobTask', data.regiontrafficKey.missionName,
        CoopDataClient.roomSetting.owneridentifier)
    if data.regiontrafficKey.missionName == "fixHouseBoard" then
        TriggerServerEvent('insane-electrician:server:switchFixedHouse',
            CoopDataClient.roomSetting.owneridentifier,
            coords)
    elseif data.regiontrafficKey.missionName == "fixTrafo" then
        TriggerServerEvent('insane-electrician:server:switchFixedTrafo',
            CoopDataClient.roomSetting.owneridentifier,
            coords)
    elseif data.regiontrafficKey.missionName == "phonePole" then
        TriggerServerEvent('insane-electrician:server:switchFixedPhonePole',
            CoopDataClient.roomSetting.owneridentifier,
            coords)
    end
end)
...
RegisterNUICallback('closeNUIHouse', function(data)
    local coords = data.regiontrafficKey.coords
    SetNuiFocus(false, false)
    TriggerServerEvent('insane-electrician:server:changeTrafoModal',
...
RegisterNetEvent('insane-electrician:client:sendNotification', function(message, type)
...
RegisterNetEvent('insane-electrician:client:FinishJob', function(jobData)
    CoopDataClient = jobData
    if CoopDataClient.roomSetting.carDelivered then
        ...
        SetNewWaypoint(CoopDataClient.roomSetting.Mission.regionDeliveryCoords.x,
            CoopDataClient.roomSetting.Mission.regionDeliveryCoords.y)
        TriggerServerEvent('insane-electrician:server:syncSettings', "waypoint",
            ...
            if allVehiclesInRange then
                ...
                if playerInVehicle then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('insane-electrician:server:LeaveVehicle',
                            CoopDataClient.roomSetting.owneridentifier)
                        clearBlip()
...
RegisterNetEvent('insane-electrician:client:LeaveVehicle', function(scoreAmount, awards)
    local playerData = TriggerCallback('insane-electrician:server:getPlayerData')
    local finishjobData = {
...
clearBlip = function()
    ...
    for _, blip in pairs(TrafficLamp2) do
        RemoveBlip(blip)
    end
    TrafficLamp2 = {}
...
RegisterNetEvent('insane-electrician:client:resetjob', function()
    for i = 1, #VehBlip do
        RemoveBlip(VehBlip[i])
    end
    VehBlip = {}
                ...
                for index, value in ipairs(CoopDataClient.roomSetting.Mission.regionhouseBoardCoords) do
                    ...
                    if trafodistance < 10 and not value.fixed then
                        ...
                        if trafodistance < 5 and not value.fixed then
                            ...
                            if IsControlJustPressed(0, 38) then
                                local missionPermit = TriggerCallback('insane-electrician:server:getTrafoModal', {
                                    owneridentifier = CoopDataClient.roomSetting.owneridentifier,
                                ...
                                if trafodistance < 8 and not value.fixed then
                                    ...
                                    if distanceToTrunk < 8 then
                                        ...
                                        if IsControlJustReleased(0, 38) and not liftcraft and not playerInVehicle then -- 38 is the control ID for the "E" key
                                            TriggerServerEvent('insane-electrician-server:build', 'lift', value.coords,
                                                CoopDataClient.roomSetting.owneridentifier)
                                            liftcraft = true
                                        end
                                    end
                            ...
                            if missionPermit then
                                ...
                                StartGame(settings, function(success)
                                    if success then
                                        ...
                                        TriggerServerEvent('insane-electrician:server:UpdateJobTask', 'phonePole',
                                            CoopDataClient.roomSetting.owneridentifier)
                                    else
                                        if (not IsPedClimbing(PlayerPedId())) then -- Only do animations when not climbing, as this will not work while on a ladder.
                                            PlayAnim('failMinigame')
                                ...
                                if trafodistance < 8 and not value.fixed then
                                    ...
                                    if distanceToTrunk < 8 then
                                        ...
                                        if IsControlJustReleased(0, 38) and not laddercraft and not playerInVehicle then -- 38 is the control ID for the "E" key
                                            TriggerServerEvent('insane-electrician-server:build', 'ladder', value.coords,
                                                CoopDataClient.roomSetting.owneridentifier)
                                            laddercraft = true
                                        end
                                    end
                    ...
                    if trafodistance < 5.0 and not value.fixed and onlyZdistance < 2.0 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            if missionPermit then
                                ...
                                StartGame(settings, function(success)
                                    if success then
                                        TriggerServerEvent('insane-electrician:server:switchFixedstreetLamp',
                                            CoopDataClient.roomSetting.owneridentifier, value.coords)

                                        TriggerServerEvent('insane-electrician:server:UpdateJobTask', 'fixStreetLamp',
                                            CoopDataClient.roomSetting.owneridentifier)
                                    else
                                        if (not IsPedClimbing(PlayerPedId())) then -- Only do animations when not climbing, as this will not work while on a ladder.
                                            PlayAnim('failMinigame')
...
function drawTextForTrafic()
    Citizen.CreateThread(function()
        while true do
            ...
            if CoopDataClient and CoopDataClient.roomSetting and CoopDataClient.roomSetting.startJob then
                for _, vv in ipairs(CoopDataClient.roomSetting.Mission.regiontrafficLampCoords) do
                    for _, selectedCoords in ipairs(vv.regionTrafficCoords) do
                        local coordsKey = string.format(
                            "%.2f,%.2f,%.2f",
                            selectedCoords.coords.x,
                            selectedCoords.coords.y,
                        ...
                        if vv.regionFixedTrafo.fixed and not selectedCoords.fixed then
                            local playerCoords = GetEntityCoords(PlayerPedId())
                            local trafodistance = #(playerCoords - vector3(
                                selectedCoords.coords.x,
                                selectedCoords.coords.y,
                                selectedCoords.coords.z
                            ))

                            if trafodistance < 10.0 then
                                sleep = 0
                                ...
                                DrawText3D(
                                    ...
                                    Config.Electrician["drawtext"].fixTrafficLamp
                                )

                                if trafodistance < 4.0 and IsControlJustPressed(0, 38) then
                                    local settings = {
                                        wireCount = 4,
                                        wireWidth = 0.74,
                                        maxWeldFails = 3,
                                        time = 64,
                                        missionName = "fixTrafficLamp",
                                    ...
                                    StartGame(settings, function(success)
                                        if success then
                                            TriggerServerEvent(
                                                "insane-electrician:server:trafficLightsFixed",
                                                CoopDataClient.roomSetting.owneridentifier,
                                                selectedCoords.coords
                                            )

                                            TriggerServerEvent(
                                                "insane-electrician:server:UpdateJobTask",
                                                "fixTrafficLamp",
                                                CoopDataClient.roomSetting.owneridentifier
                                            )
                                        else
                                            if not IsPedClimbing(PlayerPedId()) then
                                                PlayAnimfailMinigame()
                                                Wait(2000)
                                            end
...
function addBlipsFunctions(coords, label, jobName)
    ...
    elseif jobName == 'fixTrafficLamp2' then
        local coordsKey = string.format("%.2f,%.2f,%.2f", coords.x, coords.y, coords.z)
        TrafficLamp2[coordsKey] = blip
    end
end
...
RegisterNUICallback('fixWirings', function(data)
    ...
    TriggerServerEvent('insane-electrician:server:UpdateJobTask', data.missionName,
        CoopDataClient.roomSetting.owneridentifier)
    TriggerServerEvent('insane-electrician:server:trafficLightsFixed', CoopDataClient.roomSetting.owneridentifier,
        data.coords)
end)
...
-- LİFT--
RegisterNetEvent("insane-electrician:createLift", function(data)
    local input = 4.0
    ...
    TriggerServerEvent("insane-electrician:spawnLift", coords, GetEntityHeading(ped) - 180.0, tonumber(input))
end)
...
RegisterNetEvent("insane-electrician:updateLift", function(id, data)
    ...
    CreateThread(function()
        ...
        while ControllingLift do
            ...
            if IsControlJustPressed(1, controls.up) then
                TriggerServerEvent("insane-electrician:moveLift", index, "up", true, GetEntityCoords(lift.lift).z)
            elseif IsControlJustReleased(1, controls.up) then
                TriggerServerEvent("insane-electrician:moveLift", index, "up", false, GetEntityCoords(lift.lift).z)
            end
            if IsControlJustPressed(1, controls.down) then
                TriggerServerEvent("insane-electrician:moveLift", index, "down", true, GetEntityCoords(lift.lift).z)
            elseif IsControlJustReleased(1, controls.down) then
                TriggerServerEvent("insane-electrician:moveLift", index, "down", false, GetEntityCoords(lift.lift).z)
            end
            Wait(0)
        end
...
function RemoveLift(index)
    local success = TriggerCallback('insane-electrician:canRemoveLift', index)
    if not success then return end
    local success = deleteLift()
    if not success then return end
    ladder = false
    liftcraft = false
    TriggerServerEvent("insane-electrician:removeLift", index, CoopDataClient.roomSetting.owneridentifier)
end
...
RegisterNetEvent("insane-electrician:removeLift", function(id)
    DeleteLift(id)
end)
...
local lastLiftInfo = nil
CreateThread(function()
    while true do
        Wait(200)
        if liftInfo ~= lastLiftInfo then
            lastLiftInfo = liftInfo
            NuiMessage('LIFT_INFO', liftInfo)
        end
    end
end)
...
function RemoveLadder(key)
    ...
    local removesucces = TriggerCallback('insane-electrician:removeLadder', data)
...
function CreateLadder(coords)
    ...
    local placeLadder = TriggerCallback('insane-electrician:placeLadder', data)
...
RegisterNetEvent("insane-electrician:createLadder", function(coords)
    CreateLadder(coords)
end)

RegisterNetEvent("insane-electrician:removeLadder", function(key)
    DeleteLadder(key)
end)

RegisterNetEvent("insane-electrician:updateLadders", function(index, data)
    if index then
        Ladders[index] = data
    else
        Ladders = data
