...
AddEventHandler("esx:playerLoaded", function()
    Wait(1000)
    TriggerServerEvent('insane-electrician:loadData')
end)
...
RegisterNetEvent('insane-electrician:openMenu', function()
    if canOpen() then
        openElectricinMenu()
    end
end)
            ...
            exports['qb-target']:AddBoxZone("twelectirician" .. 1,
                ...
                1.5,
                {
                    ...
                    options = {
                        {
                            type = "client",
                            event = "insane-electrician:openMenu",
...
function FindZForCoords(x, y)
    local found, z = GetGroundZFor_3dCoord(x + 0.0, y + 0.0, 1000.0, true)
    if found then
        return z + 0.0
    end
    return 0.0
